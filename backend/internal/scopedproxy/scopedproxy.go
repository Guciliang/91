...
// NewTransport wraps direct with context-scoped proxy selection. Scoped routes
// inherit direct's transport settings for callers that need a custom TLS or
// dial configuration on both paths.
func NewTransport(direct http.RoundTripper) *Transport {
	return NewTransportWithBase(direct, direct)
}

// NewTransportWithBase wraps direct with context-scoped proxy selection.
// scopedBase provides the unproxied transport settings used when a request
// opts into a context proxy. Passing nil uses a fresh clone of
// http.DefaultTransport. This keeps a configured default drive proxy from
// becoming an unintended extra hop for a crawler-specific proxy.
func NewTransportWithBase(direct, scopedBase http.RoundTripper) *Transport {
	if direct == nil {
		direct = http.DefaultTransport
	}
	base := http.DefaultTransport.(*http.Transport).Clone()
	if transport, ok := scopedBase.(*http.Transport); ok {
...
// ConfigureTransports applies configuration to the default and scoped route
// pools. It is intended for setup before the client starts serving requests.
func (t *Transport) ConfigureTransports(configure func(*http.Transport)) {
	if t == nil || configure == nil {
		return
	}
	t.mu.Lock()
	defer t.mu.Unlock()
	if direct, ok := t.direct.(*http.Transport); ok {
		configure(direct)
	}
	configure(t.base)
	for _, transport := range t.byProxy {
		configure(transport)
	}
}
